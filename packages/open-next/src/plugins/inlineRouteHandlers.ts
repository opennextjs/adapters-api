...
export function inlineRouteHandler(updater: ContentUpdater, outputs: NextAdapterOutputs): Plugin {
	...
	return updater.updateContent("inlineRouteHandler", [
		...
		{
			filter: getCrossPlatformPathRegex(String.raw`core/routing/adapterHandler\.js$`, {
				escape: false,
			}),
		...
		{
			filter: getCrossPlatformPathRegex(String.raw`\[turbopack\]_runtime\.js$`, {
				escape: false,
			}),
