...
export function setNextjsPrebundledReact(rawPath: string) {
	...
	const routes = [...RoutesManifest.routes.static, ...RoutesManifest.routes.dynamic];

	const route = routes.find((route) => new RegExp(route.regex).test(rawPath ?? ""));
	...
	if (isApp) {
		process.env.__NEXT_PRIVATE_PREBUNDLED_REACT = NextConfig.experimental.serverActions
			? "experimental"
			: "next";
		return;
	}
