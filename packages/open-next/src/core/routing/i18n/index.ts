...
export function detectDomainLocale({
	...
	for (const domain of domains) {
		...
		if (
			...
			domain.locales?.some((locale) => lowercasedLocale === locale.toLowerCase())
		) {
			return domain;
		}
	}
 ...
 */
export function detectLocale(internalEvent: InternalEvent, i18n: i18nConfig): string {
 ...
 */
export function handleLocaleRedirect(internalEvent: InternalEvent): false | InternalResult {
	const i18n = NextConfig.i18n;
