...
export class DetachedPromiseRunner {
	...
	public async await(): Promise<void> {
		...
		const rejectedPromises = results.filter((r) => r.status === "rejected") as PromiseRejectedResult[];
...
function provideNextAfterProvider() {
	...
	const waitUntil =
		store?.waitUntil ?? ((promise: Promise<unknown>) => store?.pendingPromiseRunner.add(promise));
