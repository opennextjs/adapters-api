...
const BUCKET_REGION = process.env.BUCKET_REGION ?? process.env.AWS_REGION ?? process.env.AWS_DEFAULT_REGION;
...
const s3Loader: ImageLoader = {
	...
	load: async (key: string) => {
		...
		const response = await awsFetch(
			keyPrefix ? `${keyPrefix}/${key.replace(/^\//, "")}` : key.replace(/^\//, ""),
			{
				method: "GET",
			}
		);
		...
		if (response.status !== 200) {
			throw new FatalError(`Failed to get image. Status code: ${response.status}`);
		}
