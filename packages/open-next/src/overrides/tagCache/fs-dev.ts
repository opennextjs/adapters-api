...
const tagCache: TagCache = {
	...
	getLastModified: async (path: string, lastModified?: number) => {
		...
		const hasExpiredTag = tags.some((tagPathMapping) => {
			if (tagPathMapping.path.S === buildKey(path) && tagPathMapping.expire?.N) {
		...
		tags = unchangedTags.concat(
			newTags.map((item) => {
				const tagEntry: TagCacheMetaFile = {
					...
					...(item.stale !== undefined ? { stale: { N: `${item.stale}` } } : undefined),
					...(item.expire !== undefined ? { expire: { N: `${item.expire}` } } : undefined),
				};
				return tagEntry;
			})
		);
