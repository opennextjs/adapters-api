...
export async function createImageOptimizationBundle(options: buildHelper.BuildOptions) {
	...
	await buildHelper.esbuildAsync(
		{
			entryPoints: [path.join(options.openNextDistDir, "adapters", "image-optimization-adapter.js")],
			external: ["sharp", "next"],
	...
	fs.copyFileSync(path.join(appBuildOutputPath, ".next/BUILD_ID"), path.join(outputPath, ".next/BUILD_ID"));
