...
export async function copyTracedFiles({
	...
	const processNftFile = (nftFile: string) => {
		...
		const files: string[] = JSON.parse(readFileSync(path.join(dotNextDir, nftFile), "utf8")).files;
...
--------------------------------------------------------------------------------`);
	...
	const safeComputeCopyFilesForPage = (pagePath: string, alternativePath?: string) => {
		try {
	...
	if (hasPageDir) {
		...
		staticFiles.filter((file) => file.endsWith(".html")).forEach((file) => copyStaticFile(`server/${file}`));
	}

	// Copy .next/static/css from standalone to output dir
	// needed for optimizeCss feature to work
	if (config.experimental.optimizeCss) {
		cpSync(path.join(standaloneNextDir, "static", "css"), path.join(outputNextDir, "static", "css"), {
			recursive: true,
		});
	}
