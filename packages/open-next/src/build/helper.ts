...
export function normalizeOptions(config: OpenNextConfig, distDir: string, tempBuildDir: string) {
	...
	if (config.packageJsonPath) {
		...
		appPackageJsonPath = _pkgPath.endsWith("package.json") ? _pkgPath : path.join(_pkgPath, "./package.json");
	} else {
...
export function esbuildSync(esbuildOptions: ESBuildOptions, options: BuildOptions) {
...
export async function esbuildAsync(esbuildOptions: ESBuildOptions, options: BuildOptions) {
	const { openNextVersion, debug, minify } = options;
	...
	const result = await buildAsync({
		...
		sourcesContent: false,
		...esbuildOptions,
		external: [...(esbuildOptions.external ?? []), "next", "./open-next.config.mjs"],
		banner: {
			...esbuildOptions.banner,
			js: [
				esbuildOptions.banner?.js || "",
 ...
 */
export function removeFiles(root: string, conditionFn: (paths: TraversePath) => boolean) {
	traverseFiles(root, conditionFn, ({ absolutePath }) => fs.rmSync(absolutePath, { force: true }));
}
...
export function getBuildId(options: BuildOptions) {
	return fs.readFileSync(path.join(options.appBuildOutputPath, ".next/BUILD_ID"), "utf-8").trim();
}
...
export function copyOpenNextConfig(inputDir: string, outputDir: string, isEdge = false) {
	// Copy open-next.config.mjs
	fs.copyFileSync(
		path.join(inputDir, isEdge ? "open-next.config.edge.mjs" : "open-next.config.mjs"),
		path.join(outputDir, "open-next.config.mjs")
	);
}

export function copyEnvFile(appPath: string, packagePath: string, outputPath: string) {
 ...
 */
export async function isEdgeRuntime(overrides: DefaultOverrideOptions | undefined) {
	if (!overrides?.wrapper) {
		return false;
	}
	if (typeof overrides.wrapper === "string") {
		return ["cloudflare-edge", "cloudflare", "cloudflare-node"].includes(overrides.wrapper);
	}
 ...
 * @returns the Next.js runtime used: "webpack" or "turbopack"
 */
export function getBundlerRuntime(options: BuildOptions): "webpack" | "turbopack" {
	...
	if (
		fs.existsSync(path.join(dotNextServerPath, "chunks/[turbopack]_runtime.js")) ||
		fs.existsSync(path.join(dotNextServerPath, "chunks/ssr/[turbopack]_runtime.js"))
	) {
		return "turbopack";
	}
