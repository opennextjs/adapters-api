...
export async function createMiddleware(
	...
	const edgeMiddlewareInfo = middlewareManifest.middleware["/"] as MiddlewareInfo | undefined;
	...
	if (config.middleware?.external) {
		...
		await buildEdgeBundle({
			entrypoint: path.join(options.openNextDistDir, "adapters", "middleware.js"),
		...
		await buildEdgeBundle({
			entrypoint: path.join(options.openNextDistDir, "core", "edgeFunctionHandler.js"),
