...
export function installDependencies(outputDir: string, installOptions?: InstallOptions) {
	try {
		if (!installOptions) {
			return;
		}
		...
		const archOption = installOptions.arch ? `--arch=${installOptions.arch}` : "";
		const targetOption = installOptions.nodeVersion ? `--target=${installOptions.nodeVersion}` : "";
		const libcOption = installOptions.libc ? `--libc=${installOptions.libc}` : "";
