...
export async function bundleNextServer(outputDir: string, appPath: string, { minify = true }) {
		...
		plugins: [
			{
				...
				setup(build) {
					...
					build.onResolve({ filter: /\.\/module.compiled/ }, (args) => {
						...
						return {
							path: path.join("next/dist/compiled/next-server/", `${dir}.runtime.prod.js`),
							external: true,
						};
					});
					...
					build.onResolve({ filter: /vendored[\\/]rsc[\\/]entrypoints/ }, (args) => {
						return {
							path: args.path,
							external: true,
						};
