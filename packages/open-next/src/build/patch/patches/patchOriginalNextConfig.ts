 ...
 */
export async function patchOriginalNextConfig(options: buildHelper.BuildOptions): Promise<void> {
	if (
	...
	const manifestPath = path.join(options.appBuildOutputPath, ".next/required-server-files.json");
	...
	if (fs.existsSync(manifestPath)) {
		...
		if (manifest.config.skipTrailingSlashRedirect === undefined) {
			const { skipTrailingSlashRedirect, serverExternalPackages } = await importNextConfigFromSource(options);
			manifest.config.skipTrailingSlashRedirect = skipTrailingSlashRedirect ?? false;
			manifest.config.serverExternalPackages = serverExternalPackages ?? [];
			await fs.promises.writeFile(manifestPath, JSON.stringify(manifest, null, 2), "utf-8");
			if (fs.existsSync(manifestStandalonePath)) {
				await fs.promises.writeFile(manifestStandalonePath, JSON.stringify(manifest, null, 2), "utf-8");
			}
		}
	} else {
		throw new Error(`Could not find required-server-files.json at path: ${manifestPath}`);
	}
}
 ...
 * @returns
 */
async function importNextConfigFromSource(buildOptions: buildHelper.BuildOptions) {
