 ...
 */
export function createStaticAssets(options: buildHelper.BuildOptions, { useBasePath = false } = {}) {
	...
	fs.cpSync(path.join(appBuildOutputPath, ".next/static"), path.join(outputPath, "_next", "static"), {
		recursive: true,
	});
	...
	const appSrcPath = fs.existsSync(path.join(appPath, "src")) ? "src/app" : "app";
				...
				switch (ext) {
					...
					case ".rsc": {
						const newFilePath = path
							...
							.substring(0, path.join(outputCachePath, relativePath).length - ext.length)
							.replace(/\.prefetch$/, "")
							.concat(".cache");
