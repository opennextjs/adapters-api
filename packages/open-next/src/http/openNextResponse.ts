...
export class OpenNextNodeResponse extends Transform implements ServerResponse {
	...
	writeHead(statusCode: unknown, statusMessage?: unknown, headers?: unknown): this {
		let _headers = headers as OutgoingHttpHeaders | OutgoingHttpHeader[] | undefined;
		let _statusMessage: string | undefined;
		if (typeof statusMessage === "string") {
			_statusMessage = statusMessage;
		} else {
			_headers = statusMessage as OutgoingHttpHeaders | OutgoingHttpHeader[] | undefined;
		}
		...
		if (_headers) {
			if (Array.isArray(_headers)) {
				...
				for (let i = 0; i < _headers.length; i += 2) {
					finalHeaders[_headers[i] as string] = _headers[i + 1] as string | string[];
				}
			} else {
