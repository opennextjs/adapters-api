...
/**
 * Determines whether tag records require blocking cache regeneration.
 *
...
const tagCache: TagCache = {
	...
	async getByTag(tag) {
		try {
			...
			// We need to remove the buildId from the path
			const paths = Items?.map(({ path: { S: key } }) => key?.replace(`${NEXT_BUILD_ID}/`, "") ?? "") ?? [];
			cache?.set(tag, paths);
			return paths;
		} catch (e) {
	...
	async getLastModified(key, lastModified) {
		try {
			...
			let revalidatedTags: DynamoDBItem[];
			...
			return hasHardRevalidation(revalidatedTags, lastModified ?? 0, Date.now())
				? -1
	...
	async writeTags(tags) {
		try {
			...
			for (const paramsChunk of toInsert) {
				await Promise.all(
					paramsChunk.map(async (params) => dynamoClient.send(new BatchWriteItemCommand(params)))
				);
			}
		} catch (e) {
