...
describe("getNextConfigHeaders", () => {
	...
	it.todo("should exercise the error scenario: 'Error matching header <key> with value <value>'");
});
...
describe("handleRedirects", () => {
	...
	it("should redirect with + character and query string", () => {
		...
		expect(result.statusCode).toEqual(308);
		expect(result.headers.Location).toEqual("https://on/search?bar=hello+world&baz=new%2C+earth");
	});
});
