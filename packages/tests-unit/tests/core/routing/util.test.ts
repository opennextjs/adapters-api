...
describe("isExternal", () => {
	...
	it("returns true for absolute https url different host but same host in path", () => {
		expect(isExternal("https://absolute.com/local.com", "local.com")).toBe(true);
	});

	it("returns false for same host with port", () => {
		expect(isExternal("https://localhost:3000/path", "localhost:3000")).toBe(false);
	});

	it("returns true for different port on same hostname", () => {
		expect(isExternal("https://localhost:3001/path", "localhost:3000")).toBe(true);
	});
});
...
describe("getUrlParts", () => {
	...
	describe("external", () => {
		...
		it("returns url parts with query string", () => {
			expect(getUrlParts("http://localhost:3000/relative/path?query=1", true)).toEqual({
...
describe("convertToQueryString", () => {
	...
	it("converts mixed multi-value and single value parameters", () => {
		...
		expect(convertToQueryString(query)).toBe("?key=value1&key=value2&another=value3");
	});
	...
	it("should respect existing query encoding", () => {
		...
		expect(convertToQueryString(query)).toBe("?key=value%201&key=value2+something+else&another=value3");
	});
});
...
describe("regex", () => {
	describe("escapeRegex", () => {
		...
		])("should escape (.), (..), (...), + with _µ1_, _µ2_, _µ3_, _µ4_ - %s", (input, expected) => {
	...
	describe("unescapeRegex", () => {
		...
		])("should unescape _µ1_, _µ2_, _µ3_ with (.), (..), (...) - %s", (input, expected) => {
...
describe("fixCacheHeaderForHtmlPages", () => {
	...
	it("should set cache-control header for html page", () => {
		...
		expect(headers["cache-control"]).toBe("public, max-age=0, s-maxage=31536000, must-revalidate");
	});
...
describe("fixISRHeaders", () => {
	...
	it("should set cache-control directive to stale-while-revalidate when x-nextjs-cache is STALE", () => {
		...
		expect(headers["cache-control"]).toBe("s-maxage=2, stale-while-revalidate=2592000");
	});
...
describe("invalidateCDNOnRequest", () => {
	...
	it("should not call invalidatePaths when x-nextjs-cache is not REVALIDATED", async () => {
		...
		expect(globalThis.cdnInvalidationHandler.invalidatePaths).not.toHaveBeenCalled();
	});
	...
	it("should call invalidatePaths when x-nextjs-cache is REVALIDATED", async () => {
		...
		expect(globalThis.cdnInvalidationHandler.invalidatePaths).toHaveBeenCalledWith([
			{
				initialPath: "/path",
				rawPath: "/path",
				resolvedRoutes: [
...
describe("normalizeLocationHeader", () => {
	it("should normalize relative location header", () => {
		const result = normalizeLocationHeader("http://localhost:3000/path", "http://localhost:3000");
		expect(result).toBe("/path");
	});

	it("should not change absolute location header", () => {
		const result = normalizeLocationHeader("https://opennext.js.org/aws", "http://localhost:3000");
		expect(result).toBe("https://opennext.js.org/aws");
	});

	it("should normalize relative location with query parameters", () => {
		const result = normalizeLocationHeader("http://localhost:3000/path?query=1", "http://localhost:3000");
	...
	it("should preserve multiple query parameters in location header", () => {
		...
		expect(result).toBe("/path?query1=value1&query1=value3&query2=value2&query2=value4&random=randomvalue");
	});

	it("should return absolute URL correctly when the base URL is different", () => {
		const result = normalizeLocationHeader("https://example.com/path?query=1", "http://localhost:3000");
