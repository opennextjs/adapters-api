	...
	it("Should parse the headers with cookies + expires", async () => {
		...
		expect(response?.headers).toStrictEqual({
			"set-cookie": [
				{
					key: "set-cookie",
					value: "test=1; Path=/; Expires=Sun, 14 Apr 2024 22:19:07 GMT; HttpOnly; Secure; SameSite=None",
				},
				{
					key: "set-cookie",
					value: "test=2; Path=/; Expires=Sun, 14 Apr 2024 22:19:07 GMT; HttpOnly; Secure; SameSite=None",
				},
			],
		});
	});
...
describe("convertFrom", () => {
	type CloudFrontRequest = CloudFrontRequestEvent["Records"][number]["cf"]["request"];
	type CloudFrontConfig = CloudFrontRequestEvent["Records"][number]["cf"]["config"];
