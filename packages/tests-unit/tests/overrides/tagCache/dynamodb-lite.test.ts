...
describe("dynamodb-lite tagCache", () => {
	describe("getByPath", () => {
		...
		it("returns cached result on second call without re-fetching", async () => {
			mockFetch.mockResolvedValueOnce(makeJsonResponse({ Items: [{ tag: { S: BUILD_ID + "/tag1" } }] }));
	...
	describe("getByTag", () => {
		...
		it("returns cached result on second call", async () => {
			mockFetch.mockResolvedValueOnce(makeJsonResponse({ Items: [{ path: { S: BUILD_ID + "/path1" } }] }));
	...
	describe("isStale", () => {
		it("returns true when items exist beyond lastModified", async () => {
			mockFetch.mockResolvedValueOnce(makeJsonResponse({ Items: [{ revalidatedAt: { N: "99999" } }] }));
