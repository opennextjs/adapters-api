...
describe("dynamodb-nextMode tagCache", () => {
	...
	describe("hasBeenRevalidated", () => {
		...
		it("throws RecoverableError when tags.length > 100", async () => {
			...
			await expect(tagCache.hasBeenRevalidated(tags, 0)).rejects.toThrow(RecoverableError);
		});

		it("returns false when no tags were revalidated after lastModified", async () => {
			const lastModified = 50000;
			const revalidatedAt = 30000;
				...
				makeJsonResponse({
					Responses: {
						[TABLE_NAME]: [
							{
								tag: { S: buildKey("tag1") },
								revalidatedAt: { N: String(lastModified - 1) },
		...
		it("returns false for a tag absent from DynamoDB", async () => {
			mockFetch.mockResolvedValueOnce(makeJsonResponse({ Responses: { [TABLE_NAME]: [] } }));
		...
		it("throws RecoverableError when the response status is not 200", async () => {
			...
			await expect(tagCache.hasBeenRevalidated(["tag1"], 0)).rejects.toThrow(RecoverableError);
		});
	});
	...
	describe("isStale", () => {
		...
		it("throws RecoverableError when the response status is not 200", async () => {
			...
			await expect(tagCache.isStale(["tag1"], 0)).rejects.toThrow(RecoverableError);
		});
	});
	...
	describe("writeTags", () => {
		it("writes string tags using the tag as both path and tag key", async () => {
			...
			const items = body.RequestItems[TABLE_NAME].map((r: any) => r.PutRequest.Item);
