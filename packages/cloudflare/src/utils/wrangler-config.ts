...
export function withoutSelfEntrypointServices(config: ResolvedConfig): {
	...
	/**
	 * `true` when the configuration was rewritten. The rewritten file is the already resolved
	 * configuration, flattened to the selected environment, so the environment must not be selected
	 * a second time when passing it on - wrangler would warn about the missing environment section.
	 */
	isFlattened: boolean;
	cleanup: () => void;
} {
	...
	if (!config.configPath || keptServices.length === services.length) {
		return { configPath: config.configPath, isFlattened: false, cleanup: () => {} };
	}
	...
	return { configPath, isFlattened: true, cleanup: () => fs.rmSync(configPath, { force: true }) };
}
