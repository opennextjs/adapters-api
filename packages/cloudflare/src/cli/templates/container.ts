...
import { getOpenNextContainer, OpenNextContainer, withRequestSignal } from "./.build/open-next-container.js";
...
export default {
	async fetch(request, env, ctx) {
		return runWithCloudflareRequestContext(request, env, ctx, async () => {
			...
			return getOpenNextContainer(env.OPEN_NEXT_CONTAINER).fetch(
				withRequestSignal(reqOrResp, request.signal)
			);
		});
	},
