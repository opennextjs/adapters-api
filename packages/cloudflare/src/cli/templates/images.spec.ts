...
describe("detectImageContentType", () => {
	...
	it("should detect SVG (<?xml prefix)", () => {
		const buffer = new Uint8Array([0x3c, 0x3f, 0x78, 0x6d, 0x6c]);
	...
	it("should detect AVIF", () => {
		const buffer = new Uint8Array([0x00, 0x00, 0x00, 0x00, 0x66, 0x74, 0x79, 0x70, 0x61, 0x76, 0x69, 0x66]);
		expect(detectImageContentType(buffer)).toBe("image/avif");
	});

	it("should detect ICO", () => {
		const buffer = new Uint8Array([0x00, 0x00, 0x01, 0x00]);
		expect(detectImageContentType(buffer)).toBe("image/x-icon");
	});

	it("should detect ICNS", () => {
		const buffer = new Uint8Array([0x69, 0x63, 0x6e, 0x73]);
		expect(detectImageContentType(buffer)).toBe("image/x-icns");
	});

	it("should detect TIFF", () => {
		const buffer = new Uint8Array([0x49, 0x49, 0x2a, 0x00]);
	...
	it("should detect JXL (short signature)", () => {
		const buffer = new Uint8Array([0xff, 0x0a]);
		expect(detectImageContentType(buffer)).toBe("image/jxl");
	});

	it("should detect JXL (long signature)", () => {
		const buffer = new Uint8Array([0x00, 0x00, 0x00, 0x0c, 0x4a, 0x58, 0x4c, 0x20, 0x0d, 0x0a, 0x87, 0x0a]);
		expect(detectImageContentType(buffer)).toBe("image/jxl");
	});

	it("should detect HEIC", () => {
		const buffer = new Uint8Array([0x00, 0x00, 0x00, 0x00, 0x66, 0x74, 0x79, 0x70, 0x68, 0x65, 0x69, 0x63]);
		expect(detectImageContentType(buffer)).toBe("image/heic");
	});

	it("should detect JP2", () => {
		const buffer = new Uint8Array([0x00, 0x00, 0x00, 0x0c, 0x6a, 0x50, 0x20, 0x20, 0x0d, 0x0a, 0x87, 0x0a]);
		expect(detectImageContentType(buffer)).toBe("image/jp2");
	});
