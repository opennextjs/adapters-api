...
const routeHasMatcher =
	...
	(redirect: RouteHas): boolean => {
		switch (redirect.type) {
			...
			case "cookie":
				return (
					!!cookies?.[redirect.key] && new RegExp(redirect.value ?? "").test(cookies[redirect.key] ?? "")
				);
			case "query":
				return query[redirect.key] && Array.isArray(redirect.value)
					? redirect.value.reduce(
							(prev, current) => prev || new RegExp(current).test(query[redirect.key] as string),
							false
						)
					: new RegExp(redirect.value ?? "").test((query[redirect.key] as string | undefined) ?? "");
			case "host":
...
function checkHas(matcher: ReturnType<typeof routeHasMatcher>, has?: RouteHas[], inverted = false) {
	return has
		? has.reduce((acc, cur) => {
...
const getParamsFromSource = (source: MatchFunction<object>) => (value: string) => {
	debug("value", value);
...
function convertMatch(match: Match, toDestination: PathFunction, destination: string) {
	if (!match) {
		return destination;
	}
...
export function getNextConfigHeaders(
	...
	for (const { headers, has, missing, regex, source, locale } of configHeaders) {
		...
		if (new RegExp(regex).test(path) && checkHas(matcher, has) && checkHas(matcher, missing, true)) {
 ...
 */
export function handleRewrites<T extends RewriteDefinition>(event: InternalEvent, rewrites: T[]) {
...
function handleRepeatedSlashRedirect(event: InternalEvent): false | InternalResult {
...
function handleTrailingSlashRedirect(event: InternalEvent): false | InternalResult {
...
export function fixDataPage(internalEvent: InternalEvent, buildId: string): InternalEvent | InternalResult {
