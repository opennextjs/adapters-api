...
export class OpenNextCdkReferenceImplementation extends Construct {
	...
	constructor(scope: Construct, id: string, props: OpenNextCdkReferenceImplementationProps) {
		...
		this.openNextOutput = JSON.parse(
			readFileSync(path.join(this.openNextBasePath, ".open-next/open-next.output.json"), "utf-8")
		) as OpenNextOutput;
	...
	private createRevalidationTable() {
		...
		if (initFn) {
			const insertFn = new CdkFunction(this, "RevalidationInsertFunction", {
				...
				code: Code.fromAsset(path.join(this.openNextBasePath, ".open-next/dynamodb-provider")),
			...
			const customResource = new AwsCustomResource(this, "RevalidationInitResource", {
				onUpdate: {
					service: "Lambda",
					action: "invoke",
					parameters: {
	...
	private createAppRunnerOrigin(_key: string, _origin: OpenNextECSOrigin): HttpOrigin {
